...
Built-in presets cover basic/high-volume variants (`basic`, `luna`, `nano`, `micro`, etc.), light/balanced variants (`light`, `terra`, `mini`, `flash`, `haiku`, `small`, etc.), heavy/strong variants, verification/second-opinion variants, parallel comparison, strict review, conservative/minimal-change passes, creative alternatives, and synthesis. GPT-5.6 Luna infers `basic`, GPT-5.6 Terra infers `light`, and GPT-5.5, GPT-5.6 Sol, and bare `gpt-5.6` infer `heavy`. Literal canonical tier words in the variant key or name (`basic`, `light`, or `heavy`) are treated as explicit overrides. Otherwise, a recognized model capability takes precedence: a Sol-backed `data-entry` alias remains `heavy`, while semantic names such as `data-entry` provide a fallback when the model tier is unknown. Specialized purpose presets such as verification and strict review remain purpose-driven. Press `i` on a preset in the wizard to see its full guidance text and the Description value it will insert.
